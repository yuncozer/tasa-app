...
description: >
  ...
  Geist Sans in lowercase weight 900 treated as graphic primitive, Geist Sans chrome (uppercase,
...
colors:
  # Tokens de La Tasa (ESTILOS.md §2). El remix automático había mapeado el acento
  # a #fbbf24, que aquí es --warning y por norma NUNCA es decorativo: se corrigió
  # a mano al verde de marca. --accent-strong no se usa: no tiene consumidor en la app.
  ink-black: "#0b1120"        # --background
  ink-black-alt: "#131c2f"    # --surface
  fire-orange: "#34d399"      # --accent  (el acento del sistema, no naranja)
  cream: "#f1f5f9"            # --foreground
  cream-muted: "#94a3b8"      # --muted
  cream-hint: "rgba(148,163,184,0.35)"
  border-dark: "#26324c"      # --border
  ink-on-orange-muted: "rgba(11,17,32,0.75)"
  ink-on-orange-hint: "rgba(11,17,32,0.55)"
  ink-on-orange-faint: "rgba(11,17,32,0.40)"
  ink-on-orange-border: "rgba(11,17,32,0.20)"
...
typography:
  ...
  label:   { fontFamily: "Geist Sans", cqw: 0.72, weight: 500, tracking: "0.14em", upper: true }
  ...
  stat-value:{ fontFamily: "Geist Sans", cqw: 5.5, weight: 900, lineHeight: 1.0, tracking: "-0.04em" }
  h1:      { fontFamily: "Geist Sans", cqw: 7.5, weight: 800, lineHeight: 0.9, tracking: "-0.03em", lower: true }
  fadelist-item:{ fontFamily: "Geist Sans", cqw: 7.5, weight: 900, lineHeight: 1.0, tracking: "-0.03em", lower: true }
  quote-mark:{ fontFamily: "Geist Sans", cqw: 10.0, weight: 900, lineHeight: 0.6 }
  fadelist-title:{ fontFamily: "Geist Sans", cqw: 10.5, weight: 900, lineHeight: 0.9, tracking: "-0.04em", lower: true }
  display: { fontFamily: "Geist Sans", cqw: 13.0, weight: 900, lineHeight: 0.88, tracking: "-0.04em", lower: true }
...
- **Fonts** — already set to **Geist Sans** (display) / **Geist Sans** (body); ignore any preset font name lingering in prose.
...
and size, not face contrast. **Geist Sans** is chrome only (numbers, kickers, tags, axis labels,
...
- **Geist Sans chrome** — uppercase, 0.14em; the `/` bullet marker; mono catalogue numbers.
...
- Keep chrome in **Geist Sans uppercase, 0.14em**; use the `/` mono bullet marker.
...
- **Geist Sans + Geist Sans via Google Fonts**; Noto Sans SC is the CJK fallback (the lowercase-display signal has no CJK equivalent — the two-register color system carries the identity, per the source).
...
```html
<style>
@font-face{font-family:"Geist Sans";font-weight:400;font-style:normal;font-display:block;src:url("assets/fonts/GeistSans-Regular.woff2") format("woff2");}
</style>
```
